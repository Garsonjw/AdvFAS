...
class CDCNpp(nn.Module):
    ...
    def __init__(self, basic_conv=Conv2d_cd, theta=0.7 ):   
        ...
        self.spoofer = nn.Linear(16 * 16, 1)
      
        self.sa1 = SpatialAttention(kernel = 7)
        self.sa2 = SpatialAttention(kernel = 5)
        self.sa3 = SpatialAttention(kernel = 3)
        self.downsample32x32 = nn.Upsample(size=(32, 32), mode='bilinear')
        self.connected_layer = nn.Linear(1024, 1)
    ...
    def forward(self, x):	    	# x [3, 256, 256]
        ...
        # spoof_output  = (torch.clamp(torch.mean(map_x, dim=(1,2)),0,1)+torch.clamp(torch.mean(map_x_1, dim=(1,2)),0,1)
        #             +torch.clamp(torch.mean(map_x_2, dim=(1,2)),0,1))/3

...
if __name__ == '__main__':
    ...
    #a=torch.load('../checkpoint/Depthnet_AWP_two_class_patch-AT_best.pt')
    a=torch.load('../checkpoint/Depthnet_two_class_patch-AT_last.pt')
    # new_state_dict = {}
    # for k,v in a.items():
    ...
    with torch.no_grad():
        model.eval()
        test_data = MyDataset('/data/home/scv7305/run/chenjiawei/fasdata/fakeglass/facepath.txt', transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean                                =[0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])]))
        ...
        for images in dataloader_test:
            ...
            map_x, map_x_1, map_x_2, spoof_output = model(images)

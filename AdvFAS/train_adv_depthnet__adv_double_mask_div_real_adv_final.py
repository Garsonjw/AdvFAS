...
from models.depthnet_double import Depthnet
...
def train_test():
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        for images, lables in dataloader_train:
            ...
            map_x, spoof_out, map_x_1, map_x_2, map_adv, map_adv_1, map_adv_2, adv_out =  model(images)
            ...
            for X in range(len(lables)):
                if lables[X] == 2:
                    map_label_adv.append(np.ones([16, 16]))
                    map_1_label_adv.append(np.ones([8, 8]))
                    map_2_label_adv.append(np.ones([4, 4]))
                    lables_adv.append(1)
                else:
                    map_label_adv.append(np.zeros([16, 16]))
                    map_1_label_adv.append(np.zeros([8, 8]))
                    map_2_label_adv.append(np.zeros([4, 4]))
                    lables_adv.append(0)
            ...
            lables_adv = torch.tensor(lables_adv, dtype=torch.float32).cuda()
            lables_live = torch.tensor(lables_live, dtype=torch.float32).cuda()
            mask = torch.tensor(mask, dtype=torch.float32).cuda()

            map_label_adv = torch.tensor(map_label_adv, dtype=torch.float32).cuda()
            map_1_label_adv = torch.tensor(map_1_label_adv, dtype=torch.float32).cuda()
            map_2_label_adv = torch.tensor(map_2_label_adv, dtype=torch.float32).cuda()
            ...
            absolute_loss_adv = nn.MSELoss(reduce = False,reduction='none')(map_adv.float(), map_label_adv)
            absolute_loss_adv = (absolute_loss_adv.mean(dim = (1,2))*mask).sum()/float(no_fake_num)

            absolute_loss_adv_1 = nn.MSELoss(reduce = False,reduction='none')(map_adv_1.float(), map_1_label_adv)
            absolute_loss_adv_1 = (absolute_loss_adv_1.mean(dim = (1,2))*mask).sum()/float(no_fake_num)

            absolute_loss_adv_2 = nn.MSELoss(reduce = False,reduction='none')(map_adv_2.float(), map_2_label_adv)
            absolute_loss_adv_2 = (absolute_loss_adv_2.mean(dim = (1,2))*mask).sum()/float(no_fake_num)
            absolute_loss_adv = absolute_loss_adv + absolute_loss_adv_1 + absolute_loss_adv_2

            # print(lables)
            # print(torch.clamp(torch.mean(map_adv, dim=(1,2)),0,1))

            spoof_loss = nn.BCELoss()(spoof_out.squeeze(-1).float(), lables_live.float())

            spoof_loss = 3*spoof_loss + absolute_loss+ absolute_loss_adv
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            for images, lables in dataloader_val:
                ...
                map_x, spoof_out, map_x_1, map_x_2, map_x_adv, map_adv_1, map_adv_2, adv_out =  model(images)
                ...
                adv_score = (torch.clamp(torch.mean(map_x_adv, dim=(1,2)),0,1) + torch.clamp(torch.mean(map_adv_1, dim=(1,2)),0,1)
                +torch.clamp(torch.mean(map_adv_2, dim=(1,2)),0,1))/3
                # print(adv_score)
                # print(lables)
                # print(spoof_out)

            ###保存分数和adv分数、原始标签
                map_score_list.append('{} {}\n'.format(map_score[0].item(), lables[0].item()))

                if lables[0] ==1:
                    adv_score_list.append('{} {}\n'.format(adv_score[0], 0))

                print(adv_score, map_score)

                if lables[0] ==0:
                ###保存分数和adv分数、原始标签
                    map_score_list.append('{} {}\n'.format(map_score[1].item(), lables[1].item()))

                    adv_score_list.append('{} {}\n'.format(adv_score[1], 1))#保存了real\fake\adv的spoof值，只保存real、adv的adv分数

            adv_filename_val = args.log+'/'+args.log +'adv_val.txt'
            with open(adv_filename_val, 'w') as file:
            ...
            ###保存分数和adv分数、原始标签
                map_score_list.append('{} {} {} {}\n'.format(map_score[0].item(), lables[0].item(), adv_score[0], 0))

                if lables[0] ==0:
                ###保存分数和adv分数、原始标签
                    adv_score_list.append('{} {} {} {}\n'.format(map_score[1].item(), lables[1].item(),adv_score[1], 1))
            ...
            val_threshold, val_ACC, val_ACER, test_ACC, test_APCER, test_BPCER, test_ACER = performances_my(score_spoof_val_filename, adv_filename_val, score_spoof_test_filename)

            val_threshold_adv, val_ACC_adv, val_ACER_adv, test_ACC_adv, test_APCER_adv, test_BPCER_adv, test_ACER_adv = performances_my_adv(score_spoof_val_filename,adv_filename_val, adv_filename_test)
            
            if val_ACC>best_val_acc:
                best_val_acc=val_ACC
                torch.save(model.state_dict(),'./checkpoint/train_adv_depthnet_double_final_best_epoch3.pt')
                print('Best test accuracy achieved on epoch: %d'%(epoch + 1))
            torch.save(model.state_dict(),'./checkpoint/train_adv_depthnet_double_final_last_epoch3.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batchsize', type=int, default=70, help='initial batchsize')  
    ...
    parser.add_argument('--log', type=str, default="depthnet_adv-double_final_epoch3", help='log and save model name')

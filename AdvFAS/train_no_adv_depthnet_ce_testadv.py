    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            if val_ACC>best_val_acc:
                ...
                torch.save(model.state_dict(),'./checkpoint/train_no_adv_'+args.model+'_testadv_final_best_'+args.attack+'casia'+'.pt')
                print('Best test accuracy achieved on epoch: %d'%(epoch + 1))
            torch.save(model.state_dict(),'./checkpoint/train_no_adv_'+args.model+'_testadv_final_last_'+args.attack+'.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='Depthnet', help='model')#Depthnet Resnet18 Resnet50
    parser.add_argument('--log', type=str, default="Depthnet_no_test_adv_PGD_casia", help='log and save model name')

...
def performances_my(map_score_val_filename, adv_filename_val, map_score_test_filename):
    ...
    with open(adv_filename_val, 'r') as file1:
    ...
    fpr,tpr,threshold = roc_curve(val_labels, val_scores, pos_label=1)
    val_err_adv, val_threshold_adv = get_err_threhold(fpr, tpr, threshold)
    
    type1 = len([s for s in data if s['map_score'] <= val_threshold_adv and s['label'] == 1])
    type2 = len([s for s in data if s['map_score'] > val_threshold_adv and s['label'] == 0])
    
    adv_val_ACC = 1-(type1 + type2) / count
    adv_val_APCER = type2 / num_fake
    adv_val_BPCER = type1 / num_real
    adv_val_ACER = (adv_val_APCER + adv_val_BPCER) / 2.0
    ...
    for line in lines:
        ...
        score = float(tokens[0])
        label = float(tokens[1])    #label = int(tokens[1])
        adv_score = float(tokens[2])
        adv_label = float(tokens[3]) 
        data.append({'map_score': score, 'label': label, 'adv_score':adv_score, 'adv_label':adv_label})
        if label==1:
            num_real += 1
        else:
            num_fake += 1
    ...
    # test based on val_threshold     
    type1 = len([s for s in data if s['map_score'] <= val_threshold and s['label'] == 1 or (s['adv_score']> val_threshold_adv and s['label'] == 1)])
    type2 = len([s for s in data if s['map_score'] > val_threshold and s['label'] == 0 and s['adv_score']<= val_threshold_adv])
...
def performances_my_adv(map_score_val_filename, adv_filename_val, map_score_test_filename):
    ...
    # test based on val_threshold     
    type1 = len([s for s in data if s['map_score'] > val_threshold and s['adv_label'] == 1 and s['adv_score']<= val_threshold_adv])
    type2 = 0
    ...
    return val_threshold_adv, val_ACC, val_ACER, test_ACC, test_APCER, test_BPCER, test_ACER

...
val_list = '/data/home/scv7305/run/yanghao/fasdata/WMCA/WMCA_dev_mini.txt'
test_list =  '/data/home/scv7305/run/yanghao/fasdata/WMCA/WMCA_eval_mini.txt'
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            for images, lables in dataloader_val:
                ...
                print(lables)
                print(spoof_out)
                print(torch.clamp(torch.mean(map_x, dim=(1,2))[0],0,1))
                map_score = (torch.clamp(torch.mean(map_x, dim=(1,2))[0],0,1)+torch.softmax(spoof_out, dim=1)[0][1].item())/2
                print(map_score)
                if lables[0] ==0:
                    map_score = (torch.clamp(torch.mean(map_x, dim=(1,2))[1],0,1)+torch.softmax(spoof_out, dim=1)[1][1].item())/2
                    print(map_score)


            #     ###保存adv的分数以及adv的标签，为了求最佳adv阈值
            #     adv_score_list.append('{} {}\n'.format(torch.softmax(spoof_out, dim=1)[0][2].item(), 0))
            #     ###保存分数和adv分数、原始标签
            #     map_score_list.append('{} {} {}\n'.format(torch.softmax(spoof_out, dim=1)[0][1].item(), torch.softmax(spoof_out, dim=1)[0][2].item(), lables[0].item()))

            #     if lables[0] == 0:
            #         adv_score_list.append('{} {}\n'.format(torch.softmax(spoof_out, dim=1)[1][2].item(), 1))
            #         map_score_list.append('{} {} {}\n'.format(torch.softmax(spoof_out, dim=1)[1][1].item(), torch.softmax(spoof_out, dim=1)[1][2].item(), lables[1].item()))
            ...
            # #最佳adv阈值
            # min_threshold, _, _, _,_ = get_threshold(adv_filename)
            
            # #把score_spoof_val_filename中判为adv的去掉
            # with open(score_spoof_val_filename, 'r') as file:
            #     lines = file.readlines()

            # no_adv_score = []
            # all_adv_lable = []#保存第一道门错误的
            # for line in lines:
            #     tokens = line.split()
            #     spoof_score = float(tokens[0])
            #     adv_score = float(tokens[1])
            #     ture_lable = int(tokens[2])
            #     if adv_score<= min_threshold:
            #         no_adv_score.append('{} {}\n'.format(spoof_score, ture_lable))
            #     if adv_score> min_threshold:
            #         all_adv_lable.append('{}\n'.format(ture_lable))

            # #保存验证集排除adv后的score和lable    
            # score_spoof_val_noadv_filename = args.log+'/'+args.log+'score_spoof_val_noadv.txt'
            # with open(score_spoof_val_noadv_filename, 'w') as file:
            #     file.writelines(no_adv_score)

            # score_spoof_val_all_adv_filename = args.log+'/'+args.log+'score_spoof_val_all_adv.txt'
            # with open(score_spoof_val_all_adv_filename, 'w') as file:
            #     file.writelines(all_adv_lable)
            ...
            for images, lables in dataloader_test:
                ...
                print(lables)
                map_score = (torch.clamp(torch.mean(map_x, dim=(1,2)),0,1)+torch.softmax(spoof_out, dim=1)[0][1].item())/2
                print(map_score)
                if lables[0] ==0:
                    map_score = (torch.clamp(torch.mean(map_x, dim=(1,2)),0,1)+torch.softmax(spoof_out, dim=1)[1][1].item())/2
                    print(map_score)
                #map_score = spoof_out
            #     if torch.softmax(spoof_out, dim=1)[0][2]<=min_threshold:

            #         score_spoof = torch.softmax(spoof_out, dim=1)[0][1]
            #         #print("常规:{}\n".format(torch.sigmoid(spoof_out)[0]))
            #         map_score_list.append('{} {}\n'.format(score_spoof.item(), lables[0].item()))
            #     else:
            #         all_adv_lable.append('{}\n'.format(lables[0].item()))

            #     if lables[0] ==0:
            #         #print("adv:{}\n".format(torch.sigmoid(spoof_out)[1]))
            #         if torch.softmax(spoof_out, dim=1)[1][2]<min_threshold:
            #             # print(torch.sigmoid(spoof_out)[1])
            #             # print('fail-1')
            #             score_adv = torch.softmax(spoof_out, dim=1)[1][1]
            ...
            # score_spoof_test_all_adv_filename = args.log+'/'+args.log+'score_spoof_test_all_adv.txt'
            # with open(score_spoof_test_all_adv_filename, 'w') as file:
            #     file.writelines(all_adv_lable)    
            ...
            val_threshold, test_threshold, val_ACC, val_ACER, test_ACC, test_APCER, test_BPCER, test_ACER, test_ACER_test_threshold = performances(score_spoof_val_noadv_filename, score_spoof_test_filename)
            
            if val_ACC>best_val_acc:
                best_val_acc=val_ACC
                torch.save(model.state_dict(),'./checkpoint/train_adv_depthnet_epoch_10_best.pt')
                print('Best test accuracy achieved on epoch: %d'%(epoch + 1))
            torch.save(model.state_dict(),'./checkpoint/train_adv_depthnet_epoch=10_last.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="depthnet_adv-epoch=10_test5", help='log and save model name')

...
def train_test():
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        for images, lables in dataloader_train:
            ...
            map_x, spoof_out, map_adv, adv_out =  model(images)
            ...
            absolute_loss_adv = nn.MSELoss()(map_adv.float(), map_label_adv)

            #print(absolute_loss)
            spoof_loss = nn.BCELoss(reduce = False,reduction='none')(spoof_out.squeeze(-1).float(), lables_live.float())
            spoof_loss = (spoof_loss*mask).sum()/float(live_num)
            #adv_loss = nn.BCELoss()(adv_out.squeeze(-1).float(), lables_adv.float())
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            for images, lables in dataloader_val:
                ...
                map_x, spoof_out, map_x_adv, adv_out =  model(images)
                adv_score = torch.clamp(torch.mean(map_x_adv, dim=(1,2)),0,1)
                # print(adv_score)
                # print(lables)
                # print(spoof_out)

            ###保存分数和adv分数、原始标签
                map_score_list.append('{} {}\n'.format(spoof_out[0].item(), lables[0].item()))
            ...
            adv_val_threshold, adv_test_threshold, adv_val_ACC, adv_val_ACER, adv_test_ACC, adv_test_APCER, adv_test_BPCER, adv_test_ACER, adv_test_ACER_test_threshold = performances(adv_filename_val, adv_filename_test)
            ...
            print('epoch:%d, Adv_Val:  adv_val_threshold= %.4f, adv_val_ACC= %.4f, adv_val_ACER= %.4f' % (epoch + 1, adv_val_threshold, adv_val_ACC, adv_val_ACER))
            log_file.write('\n epoch:%d, adv_Val:  adv_val_threshold= %.4f, adv_val_ACC= %.4f, adv_val_ACER= %.4f \n' % (epoch + 1, adv_val_threshold, adv_val_ACC, adv_val_ACER))
            
            print('epoch:%d, Test:  adv_ACC= %.4f, adv_APCER= %.4f, adv_BPCER= %.4f, ACER= %.4f' % (epoch + 1, adv_test_ACC, adv_test_APCER, adv_test_BPCER, adv_test_ACER))
            #print('epoch:%d, Test:  test_threshold= %.4f, test_ACER_test_threshold= %.4f\n' % (epoch + 1, test_threshold, test_ACER_test_threshold))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="depthnet_adv-double_mask_div_real adv", help='log and save model name')

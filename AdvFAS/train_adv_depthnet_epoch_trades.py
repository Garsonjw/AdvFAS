        ...
        for images, lables in dataloader_train:
            ...
            for Y in range(len(adv_lables)):
                if adv_lables[Y] ==0:
                    ...
                    adv_img = attack.forward(adv_images[Y].unsqueeze(0), torch.tensor([1]).cuda()).squeeze(0)
                    adv_imgs.append(adv_img)
                    adv_num = adv_num + 1
                elif adv_lables[Y] ==1:
                    real_image.append(images[Y])
                    real_num = real_num +1
            ...
            real_images = torch.tensor([item.cpu().detach().numpy() for item in real_image]).cuda()
            #real_images.expand(adv_num, 3, 128, 128)
            #print(real_images.shape)
            adv_imgss = torch.tensor([item.cpu().detach().numpy() for item in adv_imgs]).cuda()
            #print(adv_imgss.shape)
            map_label = torch.tensor(map_label, dtype=torch.float32).cuda()
            #print(mask)
            absolute_loss = nn.MSELoss()(map_x.float(), map_label)
            weight = torch.FloatTensor([1,3,1]).cuda()
            spoof_loss = nn.CrossEntropyLoss(weight =weight)(spoof_out, lables)

            _, _, _, adv_logit = model(adv_imgss)
            # _, _, _, spoof_logit = model(real_images)
            # if  adv_num % real_num ==0:
            #     spoof_logit_1 = spoof_logit.repeat(int(adv_num/real_num), 1)
            # else:
            #     spoof_logit_1 = spoof_logit.repeat(int(adv_num/real_num), 1)
            #     for Z in range(len(spoof_logit)):
            #         if Z < adv_num % real_num:
            #             # print(spoof_logit[i].unsqueeze(0).shape)
            #             # print(spoof_logit_1.shape)
            #             spoof_logit_1 = torch.cat((spoof_logit_1, spoof_logit[Z].unsqueeze(0)),dim=0)
            #     # print(spoof_logit_1.shape)
            # trade_loss = (1.0/adv_num)*torch.nn.KLDivLoss(size_average=False)(torch.nn.functional.log_softmax(adv_logit, dim=1),torch.nn.functional.softmax(spoof_logit_1, dim=1))
            trade_loss = nn.CrossEntropyLoss()(adv_logit, torch.ones([adv_logit.size(0)]).long().cuda())
            spoof_loss = 3*spoof_loss + absolute_loss
            print(trade_loss)
            print(spoof_loss) 
            spoof_loss = spoof_loss - torch.minimum(trade_loss*0.025, torch.tensor(20))
            #print(spoof_loss)
            spoof_loss = torch.maximum(spoof_loss+0.1,torch.tensor(0)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="depthnet_adv_trade", help='log and save model name')

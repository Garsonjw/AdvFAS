    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            for images, lables in dataloader_val:
                ...
                if torch.sigmoid(spoof_out)[0][2]<0.5:
                ...
                if lables[0] ==0:
                    ...
                    if torch.sigmoid(spoof_out)[1][2]<0.5:
                        print(torch.sigmoid(spoof_out)[1])
                        print('fail-1')
                        score_adv = torch.sigmoid(spoof_out)[1][1]
                        map_score_list.append('{} {}\n'.format(score_adv.item(), lables[0].item()))
            ...
            if val_ACC>best_val_acc:
                ...
                torch.save(model.state_dict(),'./checkpoint/depthnet_testadv0925.pt')
                print('Best test accuracy achieved on epoch: %d'%(epoch + 1))
            torch.save(model.state_dict(),'./checkpoint/train_no_adv_depthnet_ce_testadv0925.pt')

...
def train_test():
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        for images, lables in dataloader_train:
            ...
            #p = torch.clamp(map_x.mean(dim=(1,2)),0,1)
            #p = torch.nn.Sigmoid()(map_x.mean(dim=(1,2)))
            # print(p)
            # print(lables)
            #lables_loss = -((lables-p)**2)*(torch.log(1-p))*(1-lables)-lables*((lables-p)**2)*torch.log(p)
            #lables_loss = (map_x.mean(dim=(1,2)) - lables)**2
            #print(lables_loss)
        ...
        if epoch % epoch_test == epoch_test-1:    # test every 5 epochs  
            ...
            with torch.no_grad():
                ...
                if val_ACC>best_val_acc:
                    ...
                    torch.save(model.state_dict(),'./checkpoint/p_epoch500_ckpt_best_CDCNpp_spoofout.pt')
                    print('Best test accuracy achieved on epoch: %d'%(epoch + 1))
                torch.save(model.state_dict(),'./checkpoint/p1_ckpt_last_CDCNpp_spoofout.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="ce_problem", help='log and save model name')
